...
class RobotPlanner:
    def __init__(
        ...
        if not model_cfg.model.train_encoder and model_cfg.encoder_ckpt_path is not None:
            ...
            if 'encoder' in encoder_ckpt:
                ...
                self.dyn_model.encoder.load_state_dict(encoder_ckpt)
        ...
        # print('***********')
        # print(self.dyn_model_normalizer)
        # print('***********')
        ...
        self.env_name = "robotwin_task_env"
    ...
    def get_demo_latents(self,):
        ...
        if self.demo_dataset_path:
            if 'RoboTwin' in self.demo_dataset_config['_target_']:
                self.demo_dataset_config.zarr_path = self.demo_dataset_path
                print("***********")
                print("设置生效")
                print("***********")
                print(self.demo_dataset_path)
            # else:
        ...
        # ---- Alias mapping for normalizer keys to support different dataset conventions ----
        # RoboTwinImageDataset uses keys like 'head_cam', 'action', 'agent_pos'
        # RobotImageDynamicsModelDataset expects keys like 'head_camera', 'act', 'state'
        try:
            # Action aliases
            self.dyn_model_normalizer['action'] = self.dyn_model_normalizer['act']
            print("Mapped normalizer key 'act' -> 'action'")
            self.dyn_model_normalizer['agent_pos'] = self.dyn_model_normalizer['state']
            print("Mapped normalizer key 'state' -> 'agent_pos'")
            self.dyn_model_normalizer['head_cam'] = self.dyn_model_normalizer['head_camera']
            print("Mapped normalizer key 'head_camera' -> 'head_cam'")
            

        except Exception as e:
            print('Warning: error while mapping normalizer aliases:', e)
        # -------------------------------------------------------------------------------
        ...
        with torch.no_grad():
            for batch_idx, batch in enumerate(demo_loader):
                ...
                for view_name in self.view_names:
                    src_view = 'head_cam' if view_name == 'head_camera' else view_name
                    bs, h, _, H, W = obs[src_view].shape[:]
                    v = obs[src_view].to(self.device)
                    v = self.dyn_model_normalizer[view_name].normalize(v)
                    # v = self.img_transform(v.view(-1, 3, self.original_img_size, self.original_img_size))
                    v = self.img_transform(v.view(-1, 3, H, W))
        ...
        print("***********")
        print(self.demo_visual_latents.shape)
        print("***********")
    ...
    def prepare_obs(self, current_obs, action_shape):
        ...
        for key, meta in self.shape_obs.items():
            ...
            if key in current_obs:
                proprio_arrays.append(current_obs[key].to(self.device))
        ...
        for view_name in self.view_names:
            src_view = 'head_cam' if view_name == 'head_camera' else view_name
            v = current_obs[src_view].to(self.device)
            v = self.dyn_model_normalizer[view_name].normalize(v)
            # print('***********')
            # print(f"self.original_img_size={self.original_img_size}")
            # print('***********')
